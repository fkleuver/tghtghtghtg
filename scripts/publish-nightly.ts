...
function getCurrentVersion(lernaJsonFilePath: string) {
  ...
  assert(match !== null, `lerna.json "version" should match ${versionRegExp}`);
  const major = match[1];
  const minor = match[2];
  const patch = match[3];
...
function run(): void {
  ...
  lerna(['publish', nightlyVersion, '--npm-tag', 'dev', '--no-git-tag-version', '--no-push', '--no-verify-registry', '--no-verify-access', '--registry', 'https://registry.npmjs.org/', '-y']);
}
